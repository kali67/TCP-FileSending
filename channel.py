...
def close_program(c_sender_in, c_sender_out, c_receiver_in, c_receiver_out):
    c_sender_in.close()
    c_sender_out.close()
    c_receiver_in.close()
                    ...
                    if u < p_rate:
                        ...
                        if data_unpacked.get_magic_no() != 0x497E:
                            ...
                            if v < 0.1: #introduce bit errors
                                data_unpacked.incrementDataLen(randint(0,10))  
                            ser_packet = pickle.dumps(data_unpacked)
                            c_receiver_out.send(ser_packet)
                except EOFError:
                    print("Cannot load from empty file!Exiting...")
                    close_program(c_sender_in, c_sender_out, c_receiver_in, c_receiver_out)
                    ...
                    if u_rec < p_rate:
                        ...
                        if data_unpacked_rec.get_magic_no() != 0x497E:
                            ...
                            if v_rec < 0.1: #introduce bit errors
                                data_unpacked_rec.incrementDataLen(randint(0,10))
...
if __name__ == "__main__":
    main(10000,10001,10002,10003,10004,10058,0)
